...
/**
 * Light Plugin HTTP Driver
 */
var httpDriver = function(Core){
    var self = this;
    var model = "light";
    var db = Core.db;
    ...
    function HTTPـOptions(options) {
        return {
            hostname: options.hostname || Config.ip,
     ...
     * @return {[type]}        [description]
     */
    this.exec = function(nodeIp,point, status){
        var node = findNodeByIp(nodeIp);
        if(node.connected)
        {
            http.request(HTTPـOptions({path: "http://" + nodeIp + "/" + point.i + "/" + status}), (res) => {
    ...
    {
        console.log("start HTTP Request: Turn On");
        self.exec(point.node_ip, point.i, 1);
    };

    this.turnOff =  function(point)
    {
        console.log("start HTTP Request: Turn Off");
        self.exec(point.node_ip, point.i, 0);
    };
